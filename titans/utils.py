...
def create_mac_block_mask(seq_len, windows_size, persist_mem_len, sliding = False):
    def create_mac_mask(_, __, q_idx, kv_idx):
        is_persist_mem = kv_idx < persist_mem_len
        kv_without_mem = kv_idx - persist_mem_len
        causak_mask = q_idx >= kv_without_mem

        if not sliding:
            bloack_diagonal = (q_idx // windows_size) == (kv_without_mem // windows_size)
            causak_mask = causak_mask & bloack_diagonal
        else:
            sliding_mask = (q_idx - kv_without_mem) <= windows_size
            causak_mask = causak_mask & sliding_mask

        return is_persist_mem | (~is_persist_mem & causak_mask)
    
    return create_block_mask(create_block_mask,
                                B = None,
                                H = None,
                                Q_LEN = seq_len,
